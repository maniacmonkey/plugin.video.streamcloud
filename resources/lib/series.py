...
class Series:
    ...
    def search(self, params):
        ...
        obj = http.get("%s/search.php?view=series&title=%s&lang=%s" % (const.SERVICE_URL, d, const.LANG))
        ...
        for video in obj:
            self.item_list.append(
                DirectoryItem(
                    ...
                    "?view=series&action=seasons&title=%s" %
                    (video['urlTerm']),
                    "%s/thumbs/%s.jpg" % (const.SERVICE_URL, video['urlTerm'])
                )
            )
            ...
            if len(obj['episodes']):
                for episode in obj['episodes']:
                    self.item_list.append(
                        VideoItem(
                            "Episode %s" % episode,
                            "?view=play&title=%s&season=%s&episode=%s" %
